            ...
            <div class="col-3">
                <div class="card" style="margin-top: 20px;">
                    <div class="card-body">
                        ...
                        <h1 class="user-username"> {{ $store.state.user.username }} </h1>
                        <button type="button" class="btn btn-secondary " style="margin-right: 10px; "  data-bs-toggle="modal" :data-bs-target="'#update-user-modal-' + $store.state.user.id">修改信息</button>
                        <button type="button" class="btn btn-secondary " style="margin-right: 10px; "  data-bs-toggle="modal" >new worker</button>
                            <div class="modal fade" :id="'update-user-modal-' + $store.state.user.id" tabindex="-1">
                                <div class="modal-dialog modal-xl">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label class="form-label">用户名名称</label>
                                            <input v-model="updateInfo.username" type="text" class="form-control"  placeholder="请输入用户名名称">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">头像url</label>
                                            <input v-model="updateInfo.photo" type="text" class="form-control"  placeholder="请输入头像url">
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <div class="error-message">{{ updateInfo.error_message }}</div>
                                        <button type="button" class="btn btn-primary" @click="update_user()" >保存修改</button>
...
export default {
    ...
    setup() {
        ...
        const update_user = () => {
            $.ajax({
                url: "http://127.0.0.1:3000/api/user/account/updateuser/",
                type: "post",
                data: {
                ...
                success(resp) {
                    if (resp.error_message === "success") {
                        store.commit("updateUsername", updateInfo.username);
                        store.commit("updatePhoto", updateInfo.photo);
                        updateInfo.username = "";
                        updateInfo.photo = "";
                        Modal.getInstance('#update-user-modal-' + store.state.user.id ).hide();
                    } else {
                        updateInfo.error_message = resp.error_message;
                        setTimeout(() => {
            ...
            $.ajax({
                ...
                success(resp) {
                    if (resp.error_message === "success") {
                        ...
                        setTimeout(() => { botadd.error_message = ""; }, 2000);
